...
async fn build_question(db: &Db, id: i64, name: String, is_new: bool) -> Result<Question> {
    ...
    let rec = sqlx::query(
        r#"SELECT id AS id, source AS source, xc_id AS xc_id, recordist AS recordist,
                  ...
                  background_json AS background_json
    ...
    // Background species (XC `also`): never offer one as a wrong option, since
    // it's genuinely present in the audio.
    let bg_sci: Vec<String> = rec
        .get::<Option<String>, _>("background_json")
