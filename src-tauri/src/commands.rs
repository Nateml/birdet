...
/// Cached species blurb (description / habitat / behaviour / voice) for a bird,
/// or None if nothing has been fetched or written for it yet.
#[tauri::command]
pub async fn get_bird_info(
    state: State<'_, AppState>,
    bird_id: i64,
) -> Result<Option<crate::services::info::BirdInfo>, String> {
    crate::services::info::get_bird_info(&state.db, bird_id)
        .await
        .map_err(|e| e.to_string())
}

/// Fetch one species' blurb from iNaturalist + Wikipedia. Returns true if any
/// text was stored. Without `force`, a species that has already been tried is
/// left alone.
#[tauri::command]
pub async fn fetch_bird_info(
    app: AppHandle,
    state: State<'_, AppState>,
    bird_id: i64,
    force: Option<bool>,
) -> Result<bool, String> {
    crate::services::info::fetch_bird_info(&app, &state.db, bird_id, force.unwrap_or(false))
        .await
        .map_err(|e| e.to_string())
}

/// Fetch blurbs for every bird that doesn't have one yet (or all of them, with
/// `force`). Emits `import://progress` under the `info` stage.
#[tauri::command]
pub async fn backfill_bird_info(
    app: AppHandle,
    state: State<'_, AppState>,
    force: Option<bool>,
) -> Result<i64, String> {
    crate::services::info::backfill_bird_info(&app, &state.db, force.unwrap_or(false))
        .await
        .map_err(|e| e.to_string())
}
...
#[tauri::command]
pub async fn get_bird_image_bytes(
    app: AppHandle,
    state: State<'_, AppState>,
    bird_id: i64,
) -> Result<tauri::ipc::Response, String> {
    let name: Option<String> =
        sqlx::query_scalar("SELECT image_path FROM bird_info WHERE bird_id = ?")
            .bind(bird_id)
            .fetch_optional(&state.db.0)
            .await
            .map_err(|e| e.to_string())?
            .flatten();
    let name = name.ok_or_else(|| "No image for this bird.".to_string())?;
    // Filenames are generated by us (`<bird_id>.<ext>`), but reject any path
    // separator anyway so a bad row can't read outside the images dir.
...
/// Save the user's own notes for a bird. Untouched by a refetch.
#[tauri::command]
pub async fn set_bird_notes(
    state: State<'_, AppState>,
    bird_id: i64,
    notes: Option<String>,
) -> Result<(), String> {
    crate::services::info::set_bird_notes(&state.db, bird_id, notes)
        .await
        .map_err(|e| e.to_string())
}
